...
> The first line is using indexing to return the name of the movie in position 4. The second line uses the purrr function 'pluck'.
...
> Line 1 maps starships and then maps the length to a numeric value of its length. Line 2 directly maps the length of starships to a numeric value. Line 3 does the same as line 2, but instead of specifying the data in the map function, it pipes the data into the map function. 
...
> There is a positive relationship between the total number of ships in a Star Wars film and the number of hyperdrive ships in a Star Wars film.
...
> General Grievous is the heaviest character.
